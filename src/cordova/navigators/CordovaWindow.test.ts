...
import { CordovaWindow } from "./CordovaWindow";
import type { NavigateParams } from "../../navigators/IWindow";

const flushPromises = () => new Promise(process.nextTick);

describe("CordovaWindow", () => {
    ...
    describe("hidden frame", () => {
        let frameWindow: CordovaWindow;

        beforeAll(() => {
            frameWindow = new CordovaWindow({ });
        });
    ...
    describe("close", () => {
        let subject: CordovaWindow;
        const parentRemoveChild = jest.fn();
        beforeEach(() => {
            subject = new CordovaWindow({});
    ...
    describe("navigate", () => {
        ...
        beforeAll(() => {
            ...
            } as unknown as HTMLCordovaElement),
            );
        });

        it("when frame.contentWindow is not defined should throw error", async() => {
            const frameWindow = new CordovaWindow({});
    ...
    describe("notifyParent", () => {
        ...
        it.each([
            ...
            //CordovaWindow.notifyParent(messageData.url, targetOrigin);
